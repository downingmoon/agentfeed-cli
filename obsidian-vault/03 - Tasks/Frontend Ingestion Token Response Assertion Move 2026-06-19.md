...
> [!todo]
> Candidate `worklog-detail-response-guards.contract.test.ts` handled in [[Frontend Worklog Detail Response Assertion Move 2026-06-19]]. Current next re-scan candidates: `worklog-detail-adapter.contract.test.ts`, `select-value-parsers.contract.test.ts`, and `api-error-list-contracts.contract.test.ts` at 23 pure LOC, followed by `auth-session-marker.contract.test.ts` and `public-user-strict-stats.contract.test.ts` at 22 pure LOC.
