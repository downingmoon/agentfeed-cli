...
- [x] Next re-scan candidate `api-surface-contracts.contract.test.ts` handled in [[Frontend API Surface Fixture Split 2026-06-18]]. Subsequent candidates `read-side-strict-fields.contract.test.ts` and `api-pagination-request-contracts.contract.test.ts` handled in [[Frontend Read Side Strict Field Fixture Split 2026-06-18]] and [[Frontend API Pagination Request Fixture Split 2026-06-18]]. Current next re-scan candidates: `worklog-metric-evidence.contract.test.ts` and `project-schema-variants-strict-fields.contract.test.ts` at 45 pure LOC.
