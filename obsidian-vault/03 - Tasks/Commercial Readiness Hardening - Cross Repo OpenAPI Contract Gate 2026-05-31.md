...
- CLI runtime endpoint 6개와 Frontend runtime endpoint 52개를 `{method, path}` 단위로 검증합니다.
...
- `POST /v1/comments/{comment_id}/report` — comment report UI 미연결
- `POST /v1/ingest/token/rotate` — deprecated compatibility endpoint
- `PATCH /v1/me/profile` — profile edit UI 미연결
- `POST /v1/me/username` — username setup/update UI 미연결
...
- `POST /v1/worklogs/{worklog_id}/report` — worklog report UI 미연결
...
- `node scripts/check-openapi-contract.mjs` in `agentfeed-dev` → pass
  ...
  - Client contracts checked: 58 (`cli: 6`, `frontend: 52`)
  - Classified backend-only operations: 10
