---
title: CLI Product Improvements Roadmap
aliases:
...
tags:
  ...
  - source-doc
  - roadmap
source_path: docs/todo/cli-product-improvements-roadmap.md
status: synced-copy
created: 2026-05-30
updated: 2026-05-30
---
...
> [!info] 원본 문서 복사본
> 원본: `docs/todo/cli-product-improvements-roadmap.md`. 이 파일은 Obsidian 탐색과 wikilink 연결을 위한 Vault 내부 복사본입니다.
...
- source/OMC/OMX/plugin metadata가 명시적인 USD cost(`estimated_cost_usd`, `cost_usd` 등)를 제공하는 경우에만 수집하고, `.agentfeed/config.json`의 `collection.include_estimated_cost=true`일 때만 draft/upload metrics에 노출하도록 보강
- `agentfeed doctor`의 agent signal 출력에 source별 품질 기대치, 바로 실행 가능한 `collect --source ... --explain` 명령, plugin 보강 역할, 감지 path를 추가해 수집 품질 개선 경로를 명확히 표시
- `agentfeed scan --dry-run`이 원문 secret을 노출하지 않는 상세 redaction preview를 보여주고, 일반 `scan`은 draft public field까지 실제 redaction하도록 보강

> [!success] 2026-05-30 체크포인트
> 비용은 가격표 기반으로 추정하지 않는다. 수집 source가 명시한 USD cost만 보존하고, draft/upload 노출은 opt-in 설정으로 제한한다. 관련 구현은 [[Collection System#비용 수집 정책]] 참고.

> [!success] 2026-05-30 Doctor 진단 보강
> 낮은 수집 품질을 사용자가 바로 복구할 수 있도록 `agentfeed doctor`가 Claude/Codex/Cursor/Gemini 및 OMC/OMX/Superpowers별 실행 명령과 감지 경로를 보여준다. 관련 구현은 [[Collection System#Doctor 진단 UX]] 참고.

> [!success] 2026-05-30 Privacy scan 보강
> 수동 수정된 draft에 secret이 들어가도 `agentfeed scan`이 public field를 redaction하고, `--dry-run`은 저장 없이 안전한 preview를 보여준다. 관련 구현은 [[Privacy Safety#Redaction dry-run UX]] 참고.
