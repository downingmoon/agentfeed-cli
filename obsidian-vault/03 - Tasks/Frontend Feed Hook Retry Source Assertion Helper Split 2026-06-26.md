---
title: Frontend Feed Hook Retry Source Assertion Helper Split 2026-06-26
aliases:
  - Feed hook retry source assertion helper split
status: done
tags:
  - agentfeed/frontend
  - agentfeed/contracts
...
`agentfeed-frontend/src/lib/feed-hook-retry-source-assertions.ts`가 16 pure LOC growth-risk helper였다. Runtime/UI/API 동작은 바꾸지 않고 feed hook retry source-contract 검사를 retry trigger, abort handling, request routing helpers로 분리했다.

## 변경

- `feed-hook-retry-source-assertions.ts`를 orchestration-only helper로 축소했다.
- 새 helper:
  - `feed-hook-retry-trigger-source-assertions.ts`
  - `feed-hook-abort-source-assertions.ts`
  - `feed-hook-request-source-assertions.ts`
...
- `agentfeed-frontend` `050cf70` — `Split feed hook retry assertions`
...
```text
8 src/lib/feed-hook-retry-source-assertions.ts
8 src/lib/feed-hook-retry-trigger-source-assertions.ts
8 src/lib/feed-hook-abort-source-assertions.ts
8 src/lib/feed-hook-request-source-assertions.ts
```
