...
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-26 — Post moderation-rendering/landing-preview-interaction source assertion splits threshold deploy]]

> [!success] 2026-06-26 Frontend landing preview interaction source assertion helper split
> Frontend `src/lib/landing-preview-interaction-source-assertions.ts`를 16 pure LOC helper에서 fallback, social action, share action helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 5/12/7 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 6 commits라 threshold push/deploy 예정.
> - [[Frontend Landing Preview Interaction Source Assertion Helper Split 2026-06-26]]

> [!success] 2026-06-26 Frontend moderation rendering source assertion helper split
> Frontend `src/lib/moderation-rendering-source-assertions.ts`를 16 pure LOC helper에서 accessibility, loading/empty state, report field helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 7/7/10 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
> - [[Frontend Moderation Rendering Source Assertion Helper Split 2026-06-26]]

> [!success] 2026-06-25 Project-detail-data/notifications-action source assertion threshold push/deploy
> Project detail data/notifications action source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. SSH 없음. Postgres 유지. Runtime CLI `npm ci && npm run build` 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post project-detail-data/notifications-action source assertion splits threshold deploy]]

> [!success] 2026-06-25 Frontend notifications action source assertion helper split
> Frontend `src/lib/notifications-action-source-assertions.ts`를 16 pure LOC helper에서 pending state, failure copy, failure detail helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 7/8/9 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 6 commits라 threshold push/deploy 예정.
> - [[Frontend Notifications Action Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Frontend project detail data source assertion helper split
> Frontend `src/lib/project-detail-data-source-assertions.ts`를 16 pure LOC helper에서 data loading, worklog data, identity/stats helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 8/7/9 pure LOC다. Pre-edit targeted contracts, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
> - [[Frontend Project Detail Data Source Assertion Helper Split 2026-06-25]]


> [!success] 2026-06-25 Worklog-review-privacy/shell-route source assertion threshold push/deploy
> Worklog review privacy/shell route source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. SSH 없음. Postgres 유지. Runtime CLI `npm ci && npm run build` 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post worklog-review-privacy/shell-route source assertion splits threshold deploy]]

> [!success] 2026-06-25 Frontend feed follow action source assertion helper split
> Frontend `src/lib/feed-follow-action-source-assertions.ts`를 21 pure LOC helper에서 auth-required state, optimistic/pending mutation, failure/a11y feedback helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 5/11/12 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 6 commits라 threshold push/deploy 예정.
> - [[Frontend Feed Follow Action Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Frontend worklog card navigation source assertion helper split
> Frontend `src/lib/worklog-card-navigation-source-assertions.ts`를 21 pure LOC helper에서 card component navigation, open-link CSS, shared navigation helper helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 13/6/8 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
> - [[Frontend Worklog Card Navigation Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 CLI-authorize/worklog-detail-mutation source assertion threshold push/deploy
> CLI-authorize session-storage/worklog-detail-mutation source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. SSH 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post CLI-authorize/worklog-detail-mutation source assertion splits threshold deploy]]

> [!success] 2026-06-25 Frontend worklog detail mutation source assertion helper split
> Frontend `src/lib/worklog-detail-mutation-source-assertions.ts`를 21 pure LOC helper에서 comment mutation failure, report action/auth/duplicate lock, report failure feedback helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 10/11/8 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 6 commits라 threshold push/deploy 처리 완료.
> - [[Frontend Worklog Detail Mutation Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Frontend CLI authorize session storage source assertion helper split
> Frontend `src/lib/cli-authorize-session-storage-source-assertions.ts`를 22 pure LOC helper에서 stored session metadata/recovery, visible URL cleanup, partial incoming session fail-closed helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 14/6/10 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
> - [[Frontend CLI Authorize Session Storage Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 API-boundary/project-visibility source assertion threshold push/deploy
> API-boundary rank/notification/project-visibility source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. SSH 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post API-boundary/project-visibility source assertion splits threshold deploy]]

> [!success] 2026-06-25 Frontend project visibility source assertion helper split
> Frontend `src/lib/project-visibility-source-assertions.ts`를 22 pure LOC helper에서 select parser, raw union assertion, VisibilityBadge union assertion helpers로 분리했다. Orchestrator는 19 pure LOC, 신규 helpers는 5/5/5 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 6 commits라 threshold push/deploy 처리 완료.
> - [[Frontend Project Visibility Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Frontend API-boundary rank/notification source assertion helper split
> Frontend `src/lib/api-boundary-rank-notification-source-assertions.ts`를 25 pure LOC helper에서 leaderboard enum, notification enum, username check reason helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 12/10/7 pure LOC다. Pre-edit targeted contract, post-edit targeted contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/API 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
> - [[Frontend API Boundary Rank Notification Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Profile-page/CLI-authorize source assertion threshold push/deploy
> Profile-page accessibility/CLI-authorize route source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. Codex가 이미 `trading-bot` 위에서 실행 중이라 SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post profile-page/CLI-authorize source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 Auth-shell-session/worklog-review-action source assertion threshold push/deploy
> Auth-shell-session/worklog-review-action source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. Codex가 이미 `trading-bot` 위에서 실행 중이라 SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post auth-shell-session/worklog-review-action source assertion splits threshold deploy]]

> [!success] 2026-06-25 Frontend worklog review action source assertion helper split
> Frontend `src/lib/worklog-review-action-source-assertions.ts`를 25 pure LOC helper에서 action feedback, publish/unpublish panel accessibility, typed action response guard helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 10/14/9 pure LOC다. Targeted worklog-review-assets source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 6 commits라 threshold push/deploy 처리 완료.
> - [[Frontend Worklog Review Action Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Frontend auth shell session source assertion helper split
> Frontend `src/lib/auth-shell-session-source-assertions.ts`를 26 pure LOC helper에서 fail-closed cleanup, session-expired recovery, auth.me probe/retry helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 9/14/12 pure LOC다. Targeted auth-shell source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
> - [[Frontend Auth Shell Session Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Feed-filter/brand-agent-glyph source assertion threshold push/deploy
> Feed-filter/brand-agent-glyph source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. Codex가 이미 `trading-bot` 위에서 실행 중이라 SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post feed-filter/brand-agent-glyph source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 Worklog-detail-data/API-boundary-public-user source assertion threshold push/deploy
> Worklog-detail-data/API-boundary-public-user source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. Codex가 이미 `trading-bot` 위에서 실행 중이라 SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post worklog-detail-data/API-boundary-public-user source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 Frontend brand SVG source assertion helper split
> Frontend `src/lib/brand-svg-source-assertions.ts`를 30 pure LOC helper에서 asset existence, mark/lockup, banner/glyph, error-boundary brand usage helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 9/9/14/9 pure LOC다. Targeted worklog-review-assets source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 6 commits라 threshold push/deploy 처리 완료.
> - [[Frontend Brand SVG Source Assertion Helper Split 2026-06-25]]


> [!success] 2026-06-25 Frontend discovery explore source assertion helper split
> Frontend `src/lib/discovery-explore-source-assertions.ts`를 30 pure LOC helper에서 data/load, project/tag, worklog navigation, empty/builder helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 13/11/7/11 pure LOC다. Targeted discovery-dashboard source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
...
> [!success] 2026-06-25 API-boundary project-dashboard/settings-profile source assertion threshold push/deploy
> API-boundary project-dashboard/settings-profile source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. Codex가 이미 `trading-bot` 위에서 실행 중이라 SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post API-boundary project-dashboard/settings-profile source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 Settings-shell/moderation-reports source assertion threshold push/deploy
> Settings-shell/moderation-reports source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. Codex가 이미 `trading-bot` 위에서 실행 중이라 SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post settings-shell/moderation-reports source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 Frontend settings shell source assertion helper split
> Frontend `src/lib/settings-shell-source-assertions.ts`를 32 pure LOC helper에서 shell layout, load/save isolation, identity display, integration guide helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 9/15/10/10 pure LOC다. Targeted settings source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
> - [[Frontend Settings Shell Source Assertion Helper Split 2026-06-25]]


> [!success] 2026-06-25 API-boundary visibility/worklog-detail-accessibility source assertion threshold push/deploy
> API-boundary visibility/integration + worklog-detail-accessibility source assertion split 6커밋을 push했고, 현재 서버 `trading-bot`에서 직접 `/home/ubuntu/agentfeed` runtime tree로 sync 후 backend/frontend를 force-recreate했다. Codex가 이미 `trading-bot` 위에서 실행 중이라 SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post API-boundary visibility/worklog-detail-accessibility source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 Frontend API boundary visibility/integration source assertion helper split
> Frontend `src/lib/api-boundary-visibility-integration-source-assertions.ts`를 33 pure LOC helper에서 enum parser, visibility, integration helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 14/15/14 pure LOC다. Targeted API boundary source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음. 3 commits라 threshold 미만.
...
> [!success] 2026-06-25 Frontend landing preview source assertion helper split
> Frontend `src/lib/landing-preview-source-assertions.ts`를 37 pure LOC helper에서 static data/API preview, interaction/social/share, rendering/delegation/outcome helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 16/16/14 pure LOC다. Targeted landing/settings source contracts, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
> - [[Frontend Landing Preview Source Assertion Helper Split 2026-06-25]]



> [!success] 2026-06-25 Worklog-card-list/notifications source assertion threshold push/deploy
> Worklog-card-list/notifications source assertion split 5커밋을 push했고, 현재 서버 로컬 경로 `/home/ubuntu/agentfeed`로 sync 후 backend/frontend를 force-recreate했다. `trading-bot` SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post worklog-card-list/notifications source assertion splits threshold deploy]]



> [!success] 2026-06-25 Frontend notifications source assertion helper split
> Frontend `src/lib/notifications-source-assertions.ts`를 37 pure LOC 단일 helper에서 shell, recovery/loading, list semantics, read action helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 8/15/10/16 pure LOC다. Targeted discovery-dashboard source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
> - [[Frontend Notifications Source Assertion Helper Split 2026-06-25]]

> [!success] 2026-06-25 Frontend worklog card list source assertion helper split
> Frontend `src/lib/worklog-card-list-source-assertions.ts`를 37 pure LOC 단일 helper에서 author/action, navigation, outcome, privacy-note helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 13/21/11/9 pure LOC다. Targeted worklog-card source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
> - [[Frontend Worklog Card List Source Assertion Helper Split 2026-06-25]]



> [!success] 2026-06-25 Review-public-user-assets/dashboard source assertion threshold push/deploy
> Review-public-user-assets/dashboard source assertion split 6커밋을 push했고, 현재 서버 로컬 경로 `/home/ubuntu/agentfeed`로 sync 후 backend/frontend를 force-recreate했다. `trading-bot` SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post review-public-user-assets/dashboard source assertion splits threshold deploy]]



> [!success] 2026-06-25 Frontend dashboard source assertion helper split
> Frontend `src/lib/dashboard-source-assertions.ts`를 39 pure LOC 단일 helper에서 shell/avatar, data isolation, saved worklog, recovery/loading helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 11/12/12/16 pure LOC다. Targeted discovery-dashboard source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
> - [[Frontend Dashboard Source Assertion Helper Split 2026-06-25]]



> [!success] 2026-06-25 Frontend review public user assets source assertion helper split
> Frontend `src/lib/review-public-user-assets-source-assertions.ts`를 40 pure LOC 단일 helper에서 asset metadata, public user contract/display, avatar component helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 18/15/13 pure LOC다. Targeted worklog-review assets source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
...
> [!success] 2026-06-25 Frontend project source assertion helper split
> Frontend `src/lib/project-source-assertions.ts`를 44 pure LOC 단일 helper에서 project list, create mutation, adapter/form, profile-link helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 15/17/12/9 pure LOC다. Targeted project source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
> - [[Frontend Project Source Assertion Helper Split 2026-06-25]]



> [!success] 2026-06-25 API-boundary-worklog/worklog-review-page source assertion threshold push/deploy
> API-boundary-worklog/worklog-review-page source assertion split 6커밋을 push했고, 현재 서버 로컬 경로 `/home/ubuntu/agentfeed`로 sync 후 backend/frontend를 force-recreate했다. `trading-bot` SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post API-boundary-worklog/worklog-review-page source assertion splits threshold deploy]]



> [!success] 2026-06-25 Frontend worklog review page source assertion helper split
> Frontend `src/lib/worklog-review-page-source-assertions.ts`를 50 pure LOC 단일 helper에서 guard, richer preview, privacy/public-fields, action/a11y/API helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 10/10/16/25 pure LOC다. Targeted worklog-review assets source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
> - [[Frontend Worklog Review Page Source Assertion Helper Split 2026-06-25]]



> [!success] 2026-06-25 Frontend API boundary worklog source assertion helper split
> Frontend `src/lib/api-boundary-worklog-source-assertions.ts`를 51 pure LOC 단일 helper에서 status/action, metrics/review, social/evidence, detail/collection helpers로 분리했다. Orchestrator는 10 pure LOC, 신규 helpers는 20/10/12/15 pure LOC다. Targeted API boundary source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
> - [[Frontend API Boundary Worklog Source Assertion Helper Split 2026-06-25]]



> [!success] 2026-06-25 Project-detail/shell source assertion threshold push/deploy
> Project-detail/shell source assertion split 6커밋을 push했고, 현재 서버 로컬 경로 `/home/ubuntu/agentfeed`로 sync 후 backend/frontend를 force-recreate했다. `trading-bot` SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post project-detail/shell source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 Brand/profile source assertion threshold push/deploy
> Brand/profile source assertion split 6커밋을 push했고, 현재 서버 로컬 경로 `/home/ubuntu/agentfeed`로 sync 후 backend/frontend를 force-recreate했다. `trading-bot` SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post brand/profile source assertion splits threshold deploy]]
...
> [!success] 2026-06-25 CLI-authorize/API-boundary-enum source assertion threshold push/deploy
> CLI-authorize/API-boundary-enum source assertion split 6커밋을 push했고, 현재 서버 로컬 경로 `/home/ubuntu/agentfeed`로 sync 후 backend/frontend를 force-recreate했다. `trading-bot` SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. 개발서버 HTTP IP라 hosted smoke는 `AGENTFEED_ALLOW_INSECURE_API=1`로 실행했다.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post CLI-authorize/API-boundary-enum source assertion splits threshold deploy]]


> [!success] 2026-06-25 Frontend API boundary enum source assertion helper split
> Frontend `src/lib/api-boundary-enum-source-assertions.ts`를 67 pure LOC 단일 helper에서 enum primitive/visibility/integration, leaderboard/notification/username, privacy scan helpers로 분리했다. Orchestrator는 8 pure LOC, 신규 helpers는 33/25/17 pure LOC다. Targeted API boundary source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음. 문서 commit 후 5-commit threshold 초과라 push/deploy 진행 대상.
> - [[Frontend API Boundary Enum Source Assertion Helper Split 2026-06-25]]


> [!success] 2026-06-25 Frontend CLI authorize source assertion helper split
> Frontend `src/lib/cli-authorize-source-assertions.ts`를 76 pure LOC 단일 helper에서 route/sessionStorage, auth.me/browser marker, approval/account UI, polling/transient retry, terminal cleanup/accessibility helpers로 분리했다. Orchestrator는 12 pure LOC, 신규 helpers는 25/10/16/20/19 pure LOC다. Targeted CLI authorize source contract, full contract suite, `tsc --noEmit`, production build, changed-file no-excuse/LOC scans, `git diff --check` 통과. LSP는 기존처럼 `Transport closed`라 typecheck/build/test로 대체했다. 신규 기능 없음, runtime/UI 변경 없음, 서버 배포 없음.
...
> [!success] 2026-06-25 settings/feed source assertion threshold push/deploy
> Settings/feed source assertion split 6커밋을 push했고, 현재 서버 로컬 경로 `/home/ubuntu/agentfeed`로 sync 후 backend/frontend를 force-recreate했다. `trading-bot` SSH 사용 없음. Postgres 유지. Runtime CLI build 통과, compose backend/frontend/postgres healthy, `wait-ready`, local/public readiness, metadata, hosted compatibility smoke 통과. Smoke는 개발서버 HTTP IP라 `AGENTFEED_ALLOW_INSECURE_API=1` 필요했고, runtime host frontend `node_modules` 권한을 `ubuntu:ubuntu`로 복구 후 `npm ci` 통과.
> - [[Personal Server Deploy Local Refresh 2026-06-25#2026-06-25 — Post settings/feed source assertion splits threshold deploy]]
