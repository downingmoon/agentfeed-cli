...
- `does not count browser test setup commands as executed tests` 회귀 테스트
- `recognizes direct test runner commands in Codex shell calls` 회귀 테스트
- `recognizes common wrapped test commands in Codex shell calls` 회귀 테스트
- `npm test -- tests/session-collector.test.ts --run -t "browser test setup commands|direct test runner commands|wrapped test"`
- `npm test -- tests/session-collector.test.ts --run`
- `npm test -- tests/git-draft.test.ts tests/cli-collect.test.ts --run`
- `npm run build`
- `npm test -- --run`
