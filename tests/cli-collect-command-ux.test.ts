...
describe('collect command UX and validation', () => {
  ...
  it('rejects unsupported source values before creating a draft', async () => {
    ...
    expect(failure.stderr ?? '').toContain('Supported sources: claude-code, codex, cursor, antigravity-cli, other');
    expect(failure.stderr ?? '').toContain('Tip: omit --source to let AgentFeed auto-detect Claude/Codex/Cursor/Antigravity sessions.');
    expect(failure.stderr ?? '').toContain('Did you mean: --source antigravity-cli');
    expect(failure.stderr ?? '').toContain('Run: agentfeed collect --explain');
    expect(failure.stderr ?? '').toContain('Run: agentfeed collect --source antigravity-cli --explain');
    expect(failure.stderr ?? '').toContain('Run: agentfeed collect --help');
  });
});
