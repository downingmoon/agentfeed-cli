...
- `collection.run_tests_on_collect=true`일 때 configured/auto test command를 120초 timeout으로 실행해 `tests_run` / `tests_passed` / `commands_run` / `failed_commands` metrics에 반영하고 raw test output은 draft/upload payload에 보존하지 않도록 수정
