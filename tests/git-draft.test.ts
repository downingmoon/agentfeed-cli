...
describe('git collector and drafts', () => {
  ...
  it('rejects corrupted local draft shapes with actionable field guidance', async () => {
    ...
    const cases: Array<{ name: string; mutate: (draft: Record<string, unknown>) => unknown; expected: RegExp }> = [
      ...
      { name: 'negative aggregate metric', mutate: (value) => ({ ...value, worklog: { ...(value.worklog as Record<string, unknown>), metrics: { ...((value.worklog as Record<string, unknown>).metrics as Record<string, unknown>), tokens_used: -1 } } }), expected: /AgentFeed draft is invalid.*worklog\.metrics\.tokens_used must be a non-negative number or null.*agentfeed collect/is },
      { name: 'negative agent metric', mutate: (value) => ({ ...value, worklog: { ...(value.worklog as Record<string, unknown>), metrics: { ...((value.worklog as Record<string, unknown>).metrics as Record<string, unknown>), agent_metrics: [{ agent: 'codex', tokens_used: -1 }] } } }), expected: /AgentFeed draft is invalid.*worklog\.metrics\.agent_metrics\[0\]\.tokens_used must be a non-negative number or null.*agentfeed collect/is },
      { name: 'negative timeline order', mutate: (value) => ({ ...value, worklog: { ...(value.worklog as Record<string, unknown>), timeline: [{ order: -1, title: 'Invalid negative order' }] } }), expected: /AgentFeed draft is invalid.*worklog\.timeline\[0\]\.order must be a non-negative integer.*agentfeed collect/is },
    ];
