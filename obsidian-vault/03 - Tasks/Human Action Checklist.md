...
- [x] 개인 서버 public IP 확인: `161.33.171.81`.
...
- [x] `<SERVER_IP>`를 실제 서버 IP `161.33.171.81`로 치환했다.
- [x] CLI가 바라볼 API base는 `/v1` 포함값으로 둔다: `http://161.33.171.81:18080/v1`.
- [x] Frontend가 바라볼 API URL은 `/v1` 없이 Backend origin으로 둔다: `http://161.33.171.81:18080`.
...
8. 실제 시작은 별도 단계에서 `make server-up`. 2026-06-05 실행 완료.
9. CLI에서 `AGENTFEED_ALLOW_INSECURE_API=1 AGENTFEED_API_BASE_URL=http://161.33.171.81:18080/v1 agentfeed status` 등으로 smoke. 2026-06-05 통과.
...
- [x] GitHub OAuth App Client ID/Secret을 `.env.server`에 등록.
...
- [x] `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`을 Backend `.env.server` secret으로 등록.
...
- [x] IP-only server smoke evidence를 Obsidian에 반영: [[Server IP-only Smoke Evidence 2026-06-05]].
