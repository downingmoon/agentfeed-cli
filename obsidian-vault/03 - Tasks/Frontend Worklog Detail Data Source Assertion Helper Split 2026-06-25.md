---
title: Frontend Worklog Detail Data Source Assertion Helper Split 2026-06-25
aliases:
  - Worklog detail data source assertion helper split
status: done
tags:
  - agentfeed/frontend
  - agentfeed/contracts
...
`agentfeed-frontend/src/lib/worklog-detail-data-source-assertions.ts`가 29 pure LOC growth-risk helper였다. Runtime/UI/API 동작은 바꾸지 않고 worklog detail data source-contract 문자열 검사를 retry, comment fetch, comment adapter, comment pagination helpers로 분리했다.

## 변경

- `worklog-detail-data-source-assertions.ts`를 orchestration-only helper로 축소했다.
- 새 helper:
  - `worklog-detail-retry-source-assertions.ts`
  - `worklog-detail-comment-fetch-source-assertions.ts`
  - `worklog-detail-comment-adapter-source-assertions.ts`
  - `worklog-detail-comment-pagination-source-assertions.ts`
...
```text
10 src/lib/worklog-detail-data-source-assertions.ts
12 src/lib/worklog-detail-retry-source-assertions.ts
 9 src/lib/worklog-detail-comment-fetch-source-assertions.ts
13 src/lib/worklog-detail-comment-adapter-source-assertions.ts
 8 src/lib/worklog-detail-comment-pagination-source-assertions.ts
```
