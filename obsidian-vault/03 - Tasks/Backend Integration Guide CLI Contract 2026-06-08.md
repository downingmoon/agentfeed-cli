...
- Backend `app/data/integration_guides.py`
  - Claude Code: `npm install -g agentfeed-cli` → `agentfeed login` → `agentfeed collect --source claude --explain` → `agentfeed share --dry` 순서로 정리.
...
- CLI 관련 회귀: `npm run build && npm run test -- --run tests/share.test.ts tests/cli-collect.test.ts tests/agent-discovery.test.ts tests/cli-init-setup-ux.test.ts` → `50 passed`.
