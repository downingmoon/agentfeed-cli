...
const PYTHON_JOINPATH_BINDING_TARGET = /\b(?<name>[A-Za-z_]\w*)\s*=\s*(?<baseName>[A-Za-z_]\w*)\.joinpath\((?<segments>[^)]*)\)/g;
const PYTHON_JOINPATH_SEGMENT = /\s*(?<comma>,?)\s*(['"])(?<segment>[^'"]+)\2/g;
...
function literalJoinPath(basePath: string, rawSegments: string): string | null {
  ...
  for (const match of rawSegments.matchAll(PYTHON_JOINPATH_SEGMENT)) {
    if (match.index !== cursor) return null;
    const comma = match.groups?.comma ?? '';
    if ((segments.length === 0 && comma) || (segments.length > 0 && !comma)) return null;
    const segment = match.groups?.segment;
    if (!segment || segment.includes('${')) return null;
    segments.push(segment);
    cursor = match.index + match[0].length;
...
export function pythonBoundTargets(command: string): BoundScriptTarget[] {
  ...
  PYTHON_JOINPATH_BINDING_TARGET.lastIndex = 0;
