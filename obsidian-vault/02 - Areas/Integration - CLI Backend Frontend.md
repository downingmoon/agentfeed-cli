...
Frontend Settings는 `/me/integrations` status와 `/integrations/{type}/setup-guide` guide를 함께 소비한다. Frontend compatibility runner와 dev OpenAPI gate도 이 두 endpoint와 guide response shape를 검증한다. Frontend API client는 setup guide payload를 렌더링 전에 `normalizeIntegrationGuide`로 fail-closed 검증한다.
