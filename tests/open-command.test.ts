...
describe('open command payload', () => {
  it('builds machine-readable review URL output with next actions', () => {
    expect(openJsonPayload({
      draftId: 'draft_open',
      reviewUrl: 'https://agentfeed.downingmoon.dev/worklogs/worklog_open/review',
      opened: true,
      warnings: ['ignored invalid AgentFeed API URL while opening a saved review URL: invalid URL']
    })).toEqual({
      draft_id: 'draft_open',
      review_url: 'https://agentfeed.downingmoon.dev/worklogs/worklog_open/review',
      opened: true,
      warnings: ['ignored invalid AgentFeed API URL while opening a saved review URL: invalid URL'],
  ...
  it('preserves manual-open fallback status in the JSON payload', () => {
    expect(openJsonPayload({
      ...
      reviewUrl: 'https://agentfeed.downingmoon.dev/worklogs/worklog_manual/review',
      opened: false,
      warnings: ['Review URL could not be opened automatically. Open review_url manually.']
    })).toEqual({
      draft_id: 'draft_manual',
      review_url: 'https://agentfeed.downingmoon.dev/worklogs/worklog_manual/review',
      opened: false,
      warnings: ['Review URL could not be opened automatically. Open review_url manually.'],
  ...
  it('renders human output when the review URL opens successfully', () => {
    expect(renderOpenHumanLines({
      ...
      'Draft: draft_open',
      'Review URL:',
      '  https://agentfeed.downingmoon.dev/worklogs/worklog_open/review',
      '',
      'Warnings',
  ...
  it('renders human output with a manual-open fallback when the browser launch fails', () => {
    expect(renderOpenHumanLines({
      ...
      'Draft: draft_manual',
      'Review URL:',
      '  https://agentfeed.downingmoon.dev/worklogs/worklog_manual/review',
      '',
      'Next',
      'Recommended order:',
