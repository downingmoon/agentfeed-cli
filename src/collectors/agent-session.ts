...
import { parseAntigravitySessionFile } from './agent-session-antigravity.js';
...
export async function collectAgentSessionMetrics(options: CollectAgentSessionOptions): Promise<AgentSessionMetrics | null> {
  ...
  if (options.source === 'gemini_cli') return parseAntigravitySessionFile(options.cwd, sessionFile, { since: options.since, until: options.until }, options.inferIdleGap ?? true);
  return null;
}
