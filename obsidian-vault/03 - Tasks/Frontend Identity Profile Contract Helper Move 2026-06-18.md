...
- [x] `auth-theme-social-contracts.contract.test.ts` handled in [[Frontend Auth Theme Social Contract Fixture Split 2026-06-18]]. Remaining next re-scan candidates: `cli-authorize-session.contract.test.ts` and `remaining-read-malformed-response-guards.contract.test.ts` tied at 65 pure LOC.
