---
title: Frontend Auth Shell Accessibility Source Assertion Helper Split 2026-06-25
aliases:
  - Auth shell accessibility source assertion helper split
status: done
tags:
  - agentfeed/frontend
  - agentfeed/contracts
...
# Frontend Auth Shell Accessibility Source Assertion Helper Split 2026-06-25

## 결론

`agentfeed-frontend/src/lib/auth-shell-a11y-source-assertions.ts`가 21 pure LOC growth-risk helper였다. Runtime/UI/API 동작은 바꾸지 않고 auth shell accessibility source-contract 검사를 feed popover, auth retry session banner, header a11y, sign-in fail-closed helpers로 분리했다.

## 변경

- `auth-shell-a11y-source-assertions.ts`를 orchestration-only helper로 축소했다.
- 새 helper:
  - `auth-shell-a11y-feed-source-assertions.ts`
  - `auth-shell-a11y-session-source-assertions.ts`
  - `auth-shell-a11y-header-source-assertions.ts`
  - `auth-shell-a11y-signin-source-assertions.ts`
...
```text
10 src/lib/auth-shell-a11y-source-assertions.ts
 8 src/lib/auth-shell-a11y-feed-source-assertions.ts
 6 src/lib/auth-shell-a11y-session-source-assertions.ts
10 src/lib/auth-shell-a11y-header-source-assertions.ts
 7 src/lib/auth-shell-a11y-signin-source-assertions.ts
```
...
- [x] Previous next candidate `auth-shell-a11y-source-assertions.ts` split 처리.
