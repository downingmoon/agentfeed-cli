...
> [!todo]
> No remaining `tests/cli-share.test.ts` work. The first `tests/api-hook.test.ts` extraction series is now healthy, and the first `tests/cli-status-doctor.test.ts` doctor API health extraction was completed in [[CLI Doctor API Health Test Split 2026-06-22]]. Remaining large candidates include `tests/cli-status-doctor.test.ts`, `tests/session-collector.test.ts`, and `tests/cli-collect.test.ts`; split only when a cohesive contract group is identified and baseline coverage is green.
