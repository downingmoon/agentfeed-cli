...
describe('CLI review URL handoff trust policy', () => {
  ...
  it('passes the active API and review bases into every upload handoff call', () => {
    ...
    expect((cli.match(/handoffReviewUrl\(result\.review_url/g) ?? []).length).toBe(1);
    ...
    expect((cli.match(/apiBaseUrl: creds\.api_base_url,\s*reviewBaseUrl: result\.review_base_url/g) ?? []).length).toBe(1);
    expect((cli.match(/reviewBaseUrl: result\.review_base_url/g) ?? []).length).toBe(1);
  });
});
