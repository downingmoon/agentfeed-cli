---
title: Frontend Settings Preferences Source Assertion Helper Split 2026-06-25
aliases:
  - Settings preferences source assertion helper split
status: done
tags:
  - agentfeed/frontend
  - agentfeed/contracts
...
# Frontend Settings Preferences Source Assertion Helper Split 2026-06-25

## 결론

`agentfeed-frontend/src/lib/settings-preferences-source-assertions.ts`가 19 pure LOC growth-risk helper였다. Runtime/UI/API 동작은 바꾸지 않고 settings preferences source-contract 검사를 integration/token error isolation, privacy controls, notification controls helpers로 분리했다.

## 변경

- `settings-preferences-source-assertions.ts`를 orchestration-only helper로 축소했다.
- 새 helper:
  - `settings-preferences-isolation-source-assertions.ts`
  - `settings-preferences-privacy-source-assertions.ts`
  - `settings-preferences-notification-source-assertions.ts`
...
```text
 8 src/lib/settings-preferences-source-assertions.ts
 7 src/lib/settings-preferences-isolation-source-assertions.ts
10 src/lib/settings-preferences-privacy-source-assertions.ts
10 src/lib/settings-preferences-notification-source-assertions.ts
```
