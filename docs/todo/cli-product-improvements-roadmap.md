...
작성일: 2026-05-22
근거: Claude/Antigravity 외부 자문 아티팩트(`.omx/artifacts/*agentfeed-cli-advice-20260522-004538.md`)와 현재 CLI 구현 상태.
...
- `init`에서 Claude/Codex/Antigravity/OMC/OMX/Superpowers 흔적 자동 감지
...
- 자동 draft collection은 deprecated/out-of-scope. 명시적 `share` / `collect` UX만 유지한다.
...
- Claude/Antigravity 파서에 대한 window edge case 보강 테스트
- cursor 저장 정책 UX 확정: dry-run, failed upload, manual publish 후 처리

## 2026-05-23 Claude/Antigravity 협업 후속 정리

외부 자문 공통 결론:

- 큰 기능보다 먼저 collection window와 cursor 저장 정책의 안전망을 더 촘촘히 해야 한다.
- Claude/Antigravity parser에도 window edge case 회귀 테스트가 필요하다.
- `doctor` / `init` 자동 감지가 다음 고레버리지 작업이다.
- `share --note`, 중복 draft guard, review URL clipboard는 그 다음 UX 후보이다.
- idle-gap 자동 분할, TUI rich preview, dynamic privacy rule은 아직 이르다. 자동 draft collection은 deprecated/out-of-scope로 제거한다.

이번 후속 처리:

- Claude/Antigravity collection window boundary 테스트 추가
...
- `doctor`에 Claude Code / Codex CLI / Antigravity CLI / OMC / OMX / Superpowers signal 출력 추가
- `init`에서 감지된 Codex/Antigravity agent 자동 enable
...
- Claude/Codex/Antigravity session file이 발견돼도 collection window 안 row가 없으면 high-quality evidence로 취급하지 않음
- Codex 누적 `token_count`는 `--since` 직전 baseline을 빼서 collection window 안 token delta로 집계
- Antigravity session duration은 `--since` / `--until` collection window 경계로 clamp해서 이전 작업 시간이 섞이지 않도록 집계
- windowed Codex 수집에서는 cumulative `.omx/metrics.json` token 총합이 세션 row 기반 delta를 덮어쓰지 못하게 차단
- 명시적 `--since` / `--until`이 없을 때 Claude/Codex/Antigravity session file의 30분 초과 idle gap 이후 최신 segment만 자동 수집하고 inferred `collection_window`를 draft source/fingerprint에 저장
...
- `.cursor`, `.claude`, `.codex`, `.gemini`(Antigravity), `.omc`, `.omx`, `.ai`, `.agent`, `.agents`, `.aider` 같은 agent/plugin metadata 산출물이 git changed file metrics/fingerprint를 오염시키지 않도록 제외
